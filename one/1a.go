...
expedition traditionally goes on foot. As your boats approach land, the Elves begin taking inventory of their supplies. One important consideration is food - in particular, the number of Calories each Elf is carrying (your puzzle input).
...
func OneA() int {
	...
	for _, inv := range inventories {
		...
		for _, entry := range entries {
			// for every entry in a text block, we convert it to an int,
			// then add that entry to the sum for this text block
		...
		if textBlockSum > largestSum {
			largestSum = textBlockSum
		}
