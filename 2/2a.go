...
func A() int {
	...
	for _, round := range rounds {
		...
		opponentInput, yourInput := oppenetInputToShape[shapes[0]], yourInputToShape[shapes[1]]
		scoreTotal += DetermineScore(opponentInput, yourInput)
	}

	return scoreTotal
}

func DetermineScore(opponentsInput, yourInput string) int {

	// fmt.Println(opponentsInput, yourInput)

	if opponentsInput == yourInput {
		return DRAW_SCORE + shapeToScore[yourInput]
	}

	if winMap[yourInput] == opponentsInput {
		return WIN_SCORE + shapeToScore[yourInput]
	}

	return LOSE_SCORE + shapeToScore[yourInput]
...
var oppenetInputToShape = map[string]string{
	"A": ROCK,
	"B": PAPER,
	"C": SCISSORS,
}

var yourInputToShape = map[string]string{
	"X": ROCK,
	"Y": PAPER,
	"Z": SCISSORS,
}
